...
```text
1. 不要在管理平台自行分配 ID。
2. 不要再拆出独立 TCP/UDP 隧道池。
3. 端口转发统一使用 type=portForward。
4. SOCKS5 只允许 start/stop，不允许 add/edit/delete。
5. Basic 认证只允许从服务端 Web/API 配置。
```
