...
- `v2.0.0` 是首个 `v2.x` 正式基线：已使用 Go 1.26.6 验证，并已包含 HMAC-SHA256 API
  鉴权、CSRF 与会话固化防护。
- `v2.0.1` 在上述基线上加入普通客户端控制面隔离、凭据 AES-256-GCM 非明文落盘以及
  其他完整安全加固。
- `v2.0.2` 是当前已发布生产版本，收口为 `user` + Client VerifyKey 的唯一客户端 Web 身份；
  编辑留空时安全地重新生成 VerifyKey，拒绝非空重复值，并将“NPC 能否上线”与“NPC 配置能否
  改动服务端规则”分离。
  新部署和升级环境应使用 `v2.0.2` 或更新固定版本。
...
- 自 `v2.0.2` 起，历史每 Client Web 用户名/密码不再参与鉴权或出现在 JSON；客户端
  Web 只接受固定 `user` + 非空、唯一的 Client VerifyKey。
