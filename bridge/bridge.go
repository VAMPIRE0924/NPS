	...
	for {
		if info, status, err := c.GetHealthInfo(); err != nil {
			...
			file.GetDb().JsonDb.Tasks.Range(func(key, value interface{}) bool {
				v := value.(*file.Tunnel)
				if v.Client.Id == id && file.IsPortForwardMode(v.Mode) && strings.Contains(v.Target.TargetStr, info) {
					v.Lock()
			...
			file.GetDb().JsonDb.Tasks.Range(func(key, value interface{}) bool {
				v := value.(*file.Tunnel)
				if v.Client.Id == id && file.IsPortForwardMode(v.Mode) && common.IsArrContains(v.HealthRemoveArr, info) && !common.IsArrContains(v.Target.TargetArr, info) {
					v.Lock()
	...
	for {
		...
		case common.NEW_TASK:
			if t, err := c.GetTaskInfo(); err != nil {
				...
				if len(ports) > 1 && file.IsPortForwardMode(t.Mode) && (len(ports) != len(targets)) {
					fail = true
					c.WriteAddFail()
					break loop
